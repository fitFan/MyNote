...
```typescript
            <router-link
              :to="{
                name: 'MonthReport1Index',
                query: {
                  id: scoped.row.id,
                  country: scoped.row.country
                },
              }"
            >
              详情
...
```text
1、中台开启页面缓存，将 取消頁面緩存 设置为 否
2、缓存的index页面取名字：
<script setup lang="ts">
defineOptions({
  name: 'TariffScheduleQuereyStatisticUSIndex'
})
3、\src\layout\EmptyLayout.vue 检查这个页面的监听器中 val.name 值
```
